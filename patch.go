...
func (g Grapple) Patch(image, container string, mounts []Mount) error {
	...
	if g.isPatched() {
		log.Logger().Warn("deployment already patched, rolling back first")
	...
	log.Logger().Infof("creating a configmap with deployment data")
	...
	log.Logger().Infof("extracting patch files")
	...
	log.Logger().Infof("building patch image %v:%v", pathedImageName, defaultTag)
	...
	if imageRepo != "" {
		...
		log.Logger().Infof("pushing patch image %v:%v", pathedImageName, defaultTag)
	...
	log.Logger().Infof("rendering deployment patch template")
	...
	log.Logger().Infof("patching deployment %s for development with patch", g.deployment.Name)
...
func (g Grapple) rollback(ctx context.Context) error {
	...
	for i := revision - 1; i >= 0; i-- {
		log.Logger().Infof("removing configmap %v", g.DeploymentConfigMapName())
		_, err := g.kubeCmd.DeleteConfigMap(g.DeploymentConfigMapName()).Run(ctx)
		if err != nil {
			// may not exist
			log.Logger().Warn("invalid patch state! label present but no configmap found")
		}
		log.Logger().Infof("rolling back deployment %v to revision %v", g.deployment.Name, i)
